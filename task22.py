...
def encryption(old_letter, shift):
    alphabet_big = "АБВГДЕЁЖДИЙКЛМНОПРСТУФХЦЧШЩЬЫЪЭЮЯ"
    alphabet_small = "абвгдеёждийклмнопрстуфхцчшщьыъэюя"
    flag = 0
    ltr = 0
    while True:
        if old_letter == alphabet_small[ltr]:
            new_letter = alphabet_small[ltr - shift]
            break
        if old_letter == alphabet_big[ltr]:
            new_letter = alphabet_big[ltr - shift]
            break
...
f = open("message.txt", encoding="utf-8", mode="r")
text = str()
str_number = 1
string_new = str()
letter = str()
...
while True:
    ...
    print(string)
    for i in string:
        string_new = string_new + encryption(i, str_number)
    print(string_new)
